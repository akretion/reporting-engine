...
class IrActionsReport(models.Model):
    ...
    def _build_playwright_options(
        ...
        if request and request.db:
            options["cookies"] = [{"name": "session_id", "value": request.session.sid, "domain": "odoo", "httpOnly": True, "expires": -1, "path": "/"}]
        ...
        if paperformat_id:
            ...
            if specific_paperformat_args and specific_paperformat_args.get('data-report-margin-top'):
                options["margin"]["top"] = str(specific_paperformat_args['data-report-margin-top'])
            else:
            ...
            if specific_paperformat_args and specific_paperformat_args.get('data-report-margin-bottom'):
                options["margin"]["bottom"] = str(specific_paperformat_args['data-report-margin-bottom'])
            else:
    ...
    def _run_playwright(
        ...
        try:
            ...
            with sync_playwright() as p:
                ...
                browser = p.chromium.launch(ignore_default_args=["--remote-debugging-pipe"])
                b_context = browser.new_context()
                b_context.add_cookies(print_options["cookies"])
                page = b_context.new_page()
                page.goto(url)
                page.pdf(format=print_options["format"], margin=print_options["margin"], landscape=print_options["landscape"], path=pdf_output_path)
            # BUG(franz):
            # For this to work we would need the css to be served publicly or the html to be served
            # through the same domain that the css
            # right now a cookie is used/necessary to access the css file however chrome cannot do that.
            # Cookie for cross site query and no top level request (i.e loading a css from a link tag)
            # needs to have SameSite "None" and be secured (secured cookie can only be sent through https requests)
            # There is 3 solutions:
            # - Serve the html throught the odoo:8069 endpoint
            # - make odoo:8069 https
            # - don't ask for a cookie to load css files
            # process = subprocess.run(playwright_cli, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            #
            # if process.returncode not in [0, 1]:
            #     message = _('playwright failed (error code: %s). Message: %s')
            #     _logger.warning(message, process.returncode, err[-1000:])
            #     raise UserError(message % (str(process.returncode), err[-1000:]))
            # else:
            #     if err:
            #         _logger.warning('playwright: %s' % err)
        except:
            raise
    ...
    def _render_playwright_pdf(self, report_ref, res_ids, data=None):
        ...
        if (config['test_enable'] or config['test_file']) and not self.env.context.get('force_report_rendering'):
            return self._render_qweb_html(report_ref, res_ids, data=data)

        collected_streams = self._render_playwright_pdf_prepare_streams(report_ref, data, res_ids=res_ids)
